import {Component, ViewChild, ElementRef, OnInit, ViewEncapsulation,ViewContainerRef, OnDestroy, Inject } from '@angular/core';
...
@Component({
  ...
  styles: [require('./top100.scss'),require('../css/ng2-toastr.min.scss')],
...
/*  exportToCSV(){
  ...
  exportTop100List() {
      ...
      var docDefinition = {
          content: [
            ...
            { text: this.listname+' Top 100', style: 'header' },
            {
        ...
        pdfMake.createPdf(docDefinition).download(this.listname+'-Top100.pdf');
        this.creatingpdf = false;
  }
  ...
  getLists() {
      this.loading = true;
      this.error = false;
      this._top100Service.getTop100ForList(this.listname).map(res => {
  ...
  setPDFOverlay(){
    this.overlaypdf = {'background-color' : 'Black', 'opacity': '0.7'};
  }
  ...
  removeTop100(id:Number) {
    ...
    this._top100Service.removeFromTop100(id,this.listname).subscribe(data => this.top100 = data,
    error => {
      this.unsetOverlay();
      this.showError("Could not remove Top 100, please try again!", "Error", 4000)}, 
      () =>{
        let trigger = false;
        for(var i = 0; i < this.companies.length; i++){
          ...
          if(trigger == true){
            for(var j = 0; j < this.companies[i].top100.length; j++){
              /*console.log(this.companies[i].top100[j].listName) 
              console.log(this.companies[i].top100[j].order)*/ 
              if(this.companies[i].top100[j].listName == this.listname){
                  this.companies[i].top100[j].order = this.companies[i].top100[j].order - 1;
              }        
            }
  ...
  changePosition(position:number, id:Number, current:number) {
    ...
    if(position > this.companies.length || position < 1){
      ...
      this.setOverlay();
      this._top100Service.movePosition("{\"id\":"+id+",\"order\":"+position+",\"listName\":\""+this.listname+"\"}").subscribe(data => this.top100 = data,
      error => {
      this.unsetOverlay();
      ...
      for(var j = 0; j < this.companies[current - 1].top100.length; j++){
         if(this.companies[current - 1].top100[j].listName == this.listname){
           this.companies[current -1 ].top100[j].order = position;
         }
      }
      ...
      for(var i = 0; i < this.companies.length; i ++) {
         for(var j = 0; j < this.companies[i].top100.length; j++){
           console.log(this.companies[i].top100[j].venture_id + " " + this.companies[i].top100[j].order)
         }
      }
      this.unsetOverlay();
    }
