  ...
  <table class="ng2-smart-table table" [id]="grid.getSetting('attr.id')" [ngClass]="grid.getSetting('attr.class')">
    ...
    <tr *ngFor="let row of grid.getRows()" (click)="onUserSelectRow(row)" class="ng2-smart-row" [ngClass]="{selected: row.isSelected}">
      ...
      <td *ngFor="let cell of row.getCells()">
        <div *ngIf="!cell.getRow().isInEditing && cell.getColumn().id == 'companyname' && cell.getColumn().type !== 'html'" style="font-size:20px;color:#4cbd83;font-family:freight-sans-pro,sans-serif;">{{ cell.getValue() }}</div>
        <div *ngIf="!cell.getRow().isInEditing && cell.getColumn().id == 'hqCity' && cell.getColumn().type !== 'html'" style="font-size:14px;color:#bfbfbf;font-family:freight-sans-pro,sans-serif;">{{ cell.getValue() }}</div>
